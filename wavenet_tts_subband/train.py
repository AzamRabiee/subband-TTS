...
# default parameters
LOGDIR = './logdir/train/4' # if you wanna restore
LOGDIR_ROOT = './logdir'
BATCH_SIZE = 1
TRAIN_TXT = "train_tts_short.txt" # 'eval.txt' for ground truth
CHECKPOINT_EVERY = 1#10000
NUM_STEPS = 200007#int(2e5) + 700
LEARNING_RATE = 1e-3
...
def main():
    ...
    print("last saved step: %d" % last_saved_step)
    try:
        print_loss = 0.
        ...
        for step in range(saved_global_step, args.num_steps):
            ...
            if step % PRINT_LOSS_EVERY == 0:
                ...
                print('step {:d} - loss = {:.6f}, ({:.3f} sec/step)'.format(
                    step, print_loss/PRINT_LOSS_EVERY, duration/PRINT_LOSS_EVERY))
                start_time = time.time()
                print_loss = 0.

            if step % args.checkpoint_every == 0:
                # target, predicted, lc, lc_upsampled = sess.run([net.target, net.predicted, net.lc, net.lc_upsampled])
                target, predicted = sess.run([net.target, net.predicted1])
                mat_path = logdir + "/tar_out_%d.mat" % step
                sio.savemat(mat_path, {'target': target,
                                       'output': predicted
                                       # 'target_256_%d' % step: target_256,
                                       # 'predicted1_256_%d' % step: predicted1_256,
                                       # 'predicted2_%d' % step: predicted2
                                       })
