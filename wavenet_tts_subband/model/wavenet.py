...
import numpy as np
from .ops import causal_conv, mu_law_decode, mu_law_encode, conv1d, lc_upsampling_by_repeat
...
class WaveNetModel(object):
    ...
    def __init__(self,
        ...
        self.lc_initial_channels = lc_initial_channels
        ...
        self.target = [None] * 8
        self.predicted1 = [None] * 8
        self.predicted2 = [None] * 8
        self.target_256 = [None] * 8
        self.predicted1_256 = [None] * 8
    ...
    def _create_variables(self):
        ...
        with tf.variable_scope('wavenet'):
            ...
            if hp.lc_conv_layers > 0:
                var['lc_conv_layers'] = list()
                for i in range(hp.lc_conv_layers):
                    with tf.variable_scope('lc_conv_layer{}'.format(i)):
                        layer = dict()
                        if i is 0:
                            layer['filter'] = create_variable(
                                'filter',
                                [hp.lc_fw,
                                 self.lc_initial_channels,
                                 self.local_condition_channel])
                        else:
                            layer['filter'] = create_variable(
                                'filter',
                                [hp.lc_fw,
                        ...
                        # if self.use_biases:
                            layer['filter_bias'] = create_bias_variable(
                                'filter_bias',
                                [self.local_condition_channel])
                        var['lc_conv_layers'].append(layer)

            # repeating the rest of the variables for subbands
            for subband_id in range(8):
                subband = 'd%d' % (subband_id+1)
                with tf.variable_scope('causal_layer_%s' % subband):
                    layer = dict()
                    ...
                    var['causal_layer_%s' % subband] = layer

                var['dilated_stack_%s' % subband] = list()
                with tf.variable_scope('dilated_stack_%s' % subband):
                    for i, dilation in enumerate(self.dilations):
                        with tf.variable_scope('layer{}'.format(i)):
                            ...
                            if self.local_condition_channel is not None:
                                if hp.joint_post_processing and subband_id > 0:
                                    current['lc_gate_weights'] = create_variable(
                                        'lc_gate',
                                        [1, self.local_condition_channel + self.quantization_channels,
                                         self.dilation_channels])
                            ...
                            var['dilated_stack_%s' % subband].append(current)
                    with tf.variable_scope('postprocessing_%s' % subband):
                        current = dict()
                        ...
                        var['postprocessing_%s' % subband] = current

            # if hp.joint_post_processing:
            #     with tf.variable_scope('joint_postprocessing'):
            #         layer = dict()
            #         layer['fc1'] = create_variable(
            #             'fc1',
            #             [self.quantization_channels*8,
            #              self.quantization_channels*8])
            #         layer['fc2'] = create_variable(
            #             'fc2',
            #             [self.quantization_channels*8,
            #              self.quantization_channels*8])
            #         var['joint_postprocessing'] = layer

            return var

    def _create_causal_layer(self, subband, input_batch):

        with tf.name_scope('causal_layer_%s' % subband):
            weights_filter = self.variables['causal_layer_%s' % subband]['filter']
            return causal_conv(input_batch, weights_filter, 1)

    def _create_lc_conv_layer(self, lc_batch):
        with tf.variable_scope('lc_conv_layers'):
            x = lc_batch
            h = list()
            for i in range(hp.lc_conv_layers):
                variables = self.variables['lc_conv_layers'][i]
                weights_filter = variables['filter']
                x = conv1d(x, weights_filter, self.is_training, 'conv_%d' % i)
                if hp.lc_bias:
                # if self.use_biases:
                    filter_bias = variables['filter_bias']
                    x = tf.add(x, filter_bias)
                h.append(x)
            if hp.lc_skip_conncetion:
                x = tf.add_n(h)
        return x, h

    def _create_dilation_layer(self, subband, input_batch, layer_index, dilation,
                               local_condition_batch, global_condition_batch, output_width):

        variables = self.variables['dilated_stack_%s' % subband][layer_index]
    ...
    def _generator_causal_layer(self, subband, input_batch, state_batch):
    ...
    def _generator_dilation_layer(self, subband, input_batch, state_batch, layer_index,
    ...
    def _create_network(self, subband, input_batch, local_condition_batch, global_condition_batch):
        ...
        current_layer = self._create_causal_layer(subband, current_layer)

        output_width = tf.shape(input_batch)[1] - self.receptive_field + 1

        # Add all defined dilation layers.
        with tf.name_scope('dilated_stack_%s' % subband):
            for layer_index, dilation in enumerate(self.dilations):
                with tf.name_scope('layer{}'.format(layer_index)):
                    output, current_layer = self._create_dilation_layer(
                        subband, current_layer, layer_index, dilation,
                        local_condition_batch, global_condition_batch, output_width)
                    outputs.append(output)

        with tf.name_scope('postprocessing_%s' % subband):
            # Perform (+) -> ReLU -> 1x1 conv -> ReLU -> 1x1 conv to
            # postprocess the output.
            w1 = self.variables['postprocessing_%s' % subband]['postprocess1']
            w2 = self.variables['postprocessing_%s' % subband]['postprocess2']
            if self.use_biases:
                b1 = self.variables['postprocessing_%s' % subband]['postprocess1_bias']
                b2 = self.variables['postprocessing_%s' % subband]['postprocess2_bias']
    ...
    def _create_generator(self, subband, input_batch, local_condition_batch, global_condition_batch):
        ...
        current_layer = self._generator_causal_layer(subband,
        ...
        with tf.name_scope('dilated_stack_%s' % subband):
            for layer_index, dilation in enumerate(self.dilations):
                with tf.name_scope('layer{}'.format(layer_index)):
                    ...
                    output, current_layer = self._generator_dilation_layer(subband,
        ...
        with tf.name_scope('postprocessing_%s' % subband):
            variables = self.variables['postprocessing_%s' % subband]
            # Perform (+) -> ReLU -> 1x1 conv -> ReLU -> 1x1 conv to
            # postprocess the output.
    ...
    def predict_proba_incremental(self, subband, waveform, local_condition=None,
        ...
        with tf.name_scope(name):
            ...
            subband_id = int(subband[1]) - 1
            if hp.joint_post_processing and (subband_id > 0):
                local_condition = tf.reshape(local_condition, [1, -1, self.local_condition_channel+self.quantization_channels])
            else:
                local_condition = tf.reshape(local_condition, [1, -1, self.local_condition_channel])
            gc_embedding = self._embed_gc(global_condition)

            raw_output = self._create_generator(subband, encoded, local_condition, gc_embedding)
    ...
    def loss_subband(self,
        ...
        with tf.name_scope(name):
            ...
            encoded_input = mu_law_encode(input_batch,
                                          self.quantization_channels, subband)
            encoded_input = tf.cast(encoded_input, tf.int32)
            ...
            with tf.control_dependencies([assert_op]):
                ...
                raw_output = self._create_network(subband, network_input, local_condition, gc_embedding)
                ...
                with tf.name_scope('loss'):
                    ...
                    prediction = tf.reshape(raw_output,
                                            [-1, self.quantization_channels])
                    subband_id = int(subband[1]) - 1
                    self.predicted1_256[subband_id] = prediction
                    self.target_256[subband_id] = target_output
                    # keeping one target and output for debug
                    target_sample = tf.argmax(target_output, axis=1)
                    self.target[subband_id] = mu_law_decode(target_sample, self.quantization_channels, subband)
                    prediction_sample = tf.argmax(prediction, axis=1)
                    self.predicted1[subband_id] = mu_law_decode(prediction_sample, self.quantization_channels, subband)
    ...
    def loss(self, input_batch, local_condition=None, global_condition=None, l2_regularization_strength=None, name='wavenet'):
        with tf.name_scope(name):
            gc_embedding = self._embed_gc(global_condition)

            if hp.lc_conv_layers > 0:
                local_condition, h = self._create_lc_conv_layer(local_condition)
                if hp.lc_average and not self.upsample_conditional_features: # upsampling by repeat
                    self.lc = tf.squeeze(local_condition)
                    if hp.lc_overlap:
                        lc_len = tf.shape(local_condition)
                        local_condition = lc_upsampling_by_repeat(local_condition, hp.average_window_shift)
                        mod = tf.subtract(tf.shape(input_batch)[1], tf.shape(local_condition)[1])
                        edge = tf.slice(local_condition, [0, lc_len[1] * hp.average_window_shift - 1, 0], [-1, 1, -1])
                        edge = tf.tile(edge, [1, mod, 1])
                        local_condition = tf.concat([local_condition, edge], axis=1)
                    else:
                        local_condition = lc_upsampling_by_repeat(local_condition, hp.average_window_len)
                    self.lc_upsampled = tf.squeeze(local_condition)
            if self.upsample_conditional_features:
                self.lc = tf.squeeze(local_condition)
            ...
            for i in range(8):
                subband = 'd%d' % (i + 1)
                if hp.joint_post_processing and i > 0:
                    pre_subband = 'd%d' % i
                    pre_subband_batch = mu_law_encode(input_batch[:, :, i-1],
                                                  self.quantization_channels, pre_subband)
                    pre_subband_batch = tf.cast(pre_subband_batch, tf.int32)
                    pre_subband_batch = self._one_hot(pre_subband_batch)
                    local_condition_subband = tf.concat([local_condition, pre_subband_batch], axis=2)
                else:
                    local_condition_subband = local_condition
                loss_value = loss_value + self.loss_subband(
                    subband,
                    input_batch[:, :, i],
                    local_condition=local_condition_subband,
                    gc_embedding=gc_embedding,
                    l2_regularization_strength=l2_regularization_strength,
                    name=subband)
            # # Joint post processing # TODO: for batch_size more than 1
            # if hp.joint_post_processing:
            #     # x = tf.concat(self.predicted1_256, axis=1) # it's failed; values are from 0 to 20
            #     x = [None] * 8
            #     for i in range(8):
            #         subband = 'd%d' % (i + 1)
            #         x[i] = mu_law_encode(self.predicted1[i], self.quantization_channels, subband)
            #         x[i] = tf.cast(x[i], tf.int32)
            #         x[i] = tf.squeeze(self._one_hot(x[i]))
            #     x = tf.concat(x, axis=1)
            #     w_fc1 = self.variables['joint_postprocessing']['fc1']
            #     x = tf.cast(x, tf.float32)
            #     w_fc1 = tf.cast(w_fc1, tf.float32)
            #     x = tf.nn.relu(tf.matmul(x, w_fc1))
            #     x = tf.nn.dropout(x, 0.5)
            #     w_fc2 = self.variables['joint_postprocessing']['fc2']
            #     x = tf.nn.relu(tf.matmul(x, w_fc2))
            #     x = tf.nn.dropout(x, 0.5)
            #     x = tf.reshape(x, [-1, self.quantization_channels, 8])
            #     loss = 0
            #     for i in range(8):
            #         prediction = tf.squeeze(tf.slice(x, [0, 0, i], [-1, -1, 1]))
            #         loss = loss + tf.nn.softmax_cross_entropy_with_logits(
            #             logits=prediction,
            #             labels=self.target_256[i])
            #         prediction_sample = tf.argmax(prediction, axis=1)
            #         self.predicted2[i] = mu_law_decode(prediction_sample, self.quantization_channels, subband)
            #     loss_value = loss_value + tf.reduce_mean(loss)
            return loss_value
