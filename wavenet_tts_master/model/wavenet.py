...
import numpy as np
from .ops import causal_conv, mu_law_decode, mu_law_encode, conv1d
from hparams import hparams as hp
from model.ops import lc_upsampling_by_repeat
import math
...
class WaveNetModel(object):
    ...
    def _create_variables(self):
        ...
        with tf.variable_scope('wavenet'):
            ...
            if hp.lc_causal_conv:
                with tf.variable_scope('lc_causal_layer'):
                    layer = dict()
                    layer['filter'] = create_variable(
                        'filter',
                        [self.filter_width,
    ...
    def _create_lc_causal_layer(self, lc_batch):
        with tf.name_scope('lc_causal_layer'):
            weights_filter = self.variables['lc_causal_layer']['filter']
    ...
    def _create_lc_conv_layer(self, lc_batch):
        with tf.variable_scope('lc_conv_layers'):
            ...
            for i in range(hp.lc_conv_layers):
                ...
                h.append(x)
            if hp.lc_skip_connection:
                x = tf.add_n([h[0], h[1], h[2]]) #TODO: for variable lc_conv_layers
        return x, h
    ...
    def _create_network(self, input_batch, local_condition_batch, global_condition_batch):
        ...
        if hp.lc_causal_conv:
            local_condition_batch = self._create_lc_causal_layer(local_condition_batch)
        ...
        with tf.name_scope(name):
            ...
            if hp.lc_conv_layers > 0:
                ...
                if hp.lc_average and not self.upsample_conditional_features: # upsampling by repeat
                    ...
                    if hp.lc_overlap:
                        ...
                        # mod = math.ceil(lc_len[1] * hp.average_window_shift / 256) * 256 - lc_len[1] * hp.average_window_shift
                        # self.lc_len = tf.shape(local_condition)[1]
                        # self.batch_len = tf.shape(input_batch)[0]
                        mod = tf.subtract(tf.shape(input_batch)[1], tf.shape(local_condition)[1])
                        # assert tf.less(mod, tf.convert_to_tensor(256))
            ...
            with tf.control_dependencies([assert_op]):
                ...
                with tf.name_scope('loss'):
                    ...
                    self.target = mu_law_decode(target_sample, self.quantization_channels)
                    prediction_sample = tf.argmax(prediction, axis=1)
                    self.predicted = mu_law_decode(prediction_sample, self.quantization_channels)
